...
contract Splitter{
    ...
    event FundSplitted(address, uint, address, address);
    ...
    function splitFund(address payable  _address1, address payable _address2)public payable{ 
        ...
        balances[_address2] += half;emit FundSplitted(msg.sender,msg.value, _address1, _address2);
        if(remainder > 0){
    ...
    function withdrawFund(uint _amount)public returns(bool) {
        ...
        msg.sender.transfer(_amount); emit WithdrawDone(msg.sender, _amount);
